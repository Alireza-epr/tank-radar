...
const createMockRequest = (query: Record<string, unknown>) => ({ query }) as unknown as Request;
...
describe("stationsController", () => {
  ...
  it("responds_200_with_the_station_list_for_a_valid_query", () => {
    const stations = [{ objectid: 98, street: "Bonner Str. 98", rawAddress: "Bonner Str. 98 (50677 Neustadt/Süd)", lat: 50.9, lon: 6.9 }];
    ...
    expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ success: true, entries: stations }));
  });
  ...
  it("passes_the_parsed_query_through_to_getStations", () => {
    ...
    stationsController(createMockRequest({ lat: "50.9", lon: "6.9", radius: "5" }), res);
