...
export const parseFiltersFromParams = (
  ...
  const parsedRadius =
    a_Params.radius !== undefined
      ? (Number(a_Params.radius) as TRadius)
      : undefined;
  ...
  const radius =
    ...
    VALID_RADII.includes(parsedRadius)
      ? parsedRadius
      : undefined;

  const sortBy: TSortBy =
    centerPoint &&
    a_Params.sortBy !== undefined &&
    VALID_SORT_BY.includes(a_Params.sortBy as TSortBy)
      ? (a_Params.sortBy as TSortBy)
      : "street";

  const sortDir: TSortDir =
    a_Params.sortDir !== undefined &&
    VALID_SORT_DIR.includes(a_Params.sortDir as TSortDir)
      ? (a_Params.sortDir as TSortDir)
      : "asc";

  const search =
    a_Params.search !== undefined && a_Params.search.trim().length > 0
      ? a_Params.search
      : undefined;
...
export const filtersToParams = (
  ...
  if (a_CenterPoint) {
    ...
    if (a_Filters.radius !== undefined)
      params.radius = String(a_Filters.radius);
