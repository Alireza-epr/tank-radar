...
export const createStationMarkerElement = (a_IsSelected = false, a_OnClick?: () => void): HTMLElement => {
  ...
  badge.style.backgroundColor = a_IsSelected ? STATION_BADGE_COLOR_SELECTED : STATION_BADGE_COLOR;
  badge.style.border = a_IsSelected ? `2.5px solid ${STATION_BORDER_COLOR}` : `1.5px solid ${STATION_BORDER_COLOR}`;
